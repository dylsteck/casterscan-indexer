 ...
 */
export async function insertSigner(msg) {
 ...
 */
export async function upsertSigners(signers) {
    if (signers.length === 0)
        return;
    try {
        await db
 ...
 * @param msg Hub event in JSON format
 */
export async function deleteSigner(msg) {
    const fid = msg.data.fid;
    try {
        await db
            .deleteFrom('signer')
 ...
 */
export async function deleteMessagesFromSigner(signer) {
    // Delete casts
    try {
        await db
            .deleteFrom('casts')
 ...
 */
export async function updateSigner(msg, change) {
    const fid = msg.data.fid;
