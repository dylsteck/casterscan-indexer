 ...
 */
export async function insertReaction(msg) {
    ...
    try {
        await db
            .insertInto('reaction')
            .values(reaction)
            .onConflict((oc) => oc.columns(['fid', 'target_cast', 'type']).doNothing())
 ...
 */
export async function upsertReactions(reactions) {
 ...
 * @param msg Hub event in JSON format
 */
export async function deleteReaction(msg) {
    const fid = msg.data.fid;
 ...
 */
export async function updateReaction(msg, change) {
    const fid = msg.data.fid;
    const targetCastHash = formatHash(msg.data.reactionBody.targetCastId.hash);
    try {
        await db
