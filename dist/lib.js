...
import { deleteMessagesFromSigner, deletePartOfProfile, deleteReaction, deleteSigner, deleteVerification, getLatestEvent, insertCast, insertEvent, insertProfile, insertReaction, insertSigner, insertVerification, updateCast, updateProfile, updateProfileOwner, updateReaction, updateSigner, updateVerification, } from './api/index.js';
 ...
 */
export function protobufToJson(e) {
    let event = {
        id: e.id,
        type: e.type,
        message: {},
    ...
    if (protobufs.isMergeMessageHubEvent(e)) {
        ...
        event.message = protobufs.IdRegistryEvent.toJSON(e.mergeIdRegistryEventBody.idRegistryEvent);
    }
    else if (protobufs.isMergeNameRegistryEventHubEvent(e)) {
        event.message = protobufs.NameRegistryEvent.toJSON(e.mergeNameRegistryEventBody.nameRegistryEvent);
    }
    return event;
}
/**
...
export async function handleEvent(event) {
    ...
    if (event.type === 1) {
        ...
        if (msgType === 'MESSAGE_TYPE_CAST_ADD') {
            ...
            await deleteMessagesFromSigner(formatHash(msg.data.signerRemoveBody.signer));
        }
    }
...
export async function watch() {
    ...
    result.match((stream) => {
        console.log(`Subscribed to stream ${latestEventIdFromDb ? `from event ${latestEventIdFromDb}` : ''}`);
