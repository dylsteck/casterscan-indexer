...
export function formatCasts(events) {
...
export function formatReactions(events) {
...
export function formatUserData(events, fid) {
    // Each aspect of a profile has it's own message, so we have to match the types according to data.userDataBody.type
    return {
        id: fid,
        avatar_url: events.find((user) => user.data.userDataBody.type === 'USER_DATA_TYPE_PFP')?.data.userDataBody?.value || null,
        display_name: events.find((user) => user.data.userDataBody.type === 'USER_DATA_TYPE_DISPLAY')?.data.userDataBody?.value || null,
        bio: events.find((user) => user.data.userDataBody.type === 'USER_DATA_TYPE_BIO')?.data.userDataBody?.value || null,
        url: events.find((user) => user.data.userDataBody.type === 'USER_DATA_TYPE_URL')?.data.userDataBody?.value || null,
        username: events.find((user) => user.data.userDataBody.type === 'USER_DATA_TYPE_FNAME')?.data.userDataBody?.value || null,
        updated_at: new Date(),
    };
}
export function formatVerifications(events) {
    return events.map((verification) => {
        const timestamp = fromFarcasterTime(verification.data.timestamp)._unsafeUnwrap();
        return {
            fid: verification.data.fid,
            address: formatHash(verification.data.verificationAddEthAddressBody.address),
...
export function formatSigners(events) {
...
export function breakIntoChunks(array, size) {
    const chunks = [];
