import { EthersEip712Signer, hexStringToBytes, makeCastAdd, makeCastRemove, makeReactionAdd, makeReactionRemove, makeSignerAdd, makeSignerRemove, makeUserDataAdd, makeVerificationAddEthAddress, makeVerificationEthAddressClaim, NobleEd25519Signer, } from '@farcaster/hub-nodejs';
 ...
 * @returns Cast hash
 */
async function publishCast(ed25519Signer) {
    // Make a new cast
    const cast = await makeCastAdd(protobufs.CastAddBody.create({ text: 'hello world' }), dataOptions, ed25519Signer);
 ...
 * @param ed25519Signer Signer
 */
async function likeCast(hash, ed25519Signer) {
 ...
 * @param ed25519Signer Signer
 */
async function unlikeCast(hash, ed25519Signer) {
    const reactionLikeBody = {
        type: protobufs.ReactionType.LIKE,
        targetCastId: { fid: account.fid, hash },
    };
 ...
 * @param ed25519Signer Signer
 */
async function deleteCast(hash, ed25519Signer) {
    const castRemove = await makeCastRemove(protobufs.CastRemoveBody.create({ targetHash: hash }), dataOptions, ed25519Signer);
 ...
 * @param ed25519Signer Signer
 */
async function addVerification(ed25519Signer) {
    ...
    const claimResult = makeVerificationEthAddressClaim(account.fid, addressBytes, protobufs.FarcasterNetwork.DEVNET, blockHashBytes);
    ...
    if (verificationMessage.isErr()) {
        console.error('Error making address verification', verificationMessage.error);
        return;
    }
    ...
    if (verificationAddMessage.isErr()) {
        console.error('Error submitting address verification', verificationAddMessage.error);
        return;
    }
}
/**
 ...
 * @param ed25519Signer Signer
 */
async function updatePfp(ed25519Signer) {
    ...
    const updateMessage = await client.submitMessage(userDataPfpAdd._unsafeUnwrap());
 ...
 * @param ed25519Signer Signer
 */
async function deleteSigner(ed25519Signer) {
    const eip712Signer = new EthersEip712Signer(wallet);
    const signerRemoveResult = await makeSignerRemove({ signer: (await ed25519Signer.getSignerKey())._unsafeUnwrap() }, dataOptions, eip712Signer);
...
async function sleep(seconds) {
    return new Promise((resolve) => setTimeout(resolve, seconds ? seconds * 1000 : 2000));
}
/**
